...
class SuperTimeout {
  ...
  _start() {
    ...
    if (this.timeoutID || this.startTime || this.remainingTime === 0) {
      // Prevent creation of redundant timeouts
      throw new Error(
        "SuperTimeout is either already ticking or has completed"
      ); // As long as the start() method stays private this should never show unless there is a bug in this js file
  ...
  pause() {
    // DEBUG top.console.log("pause() method of SuperTimeout fired while this.timeoutID = " + this.timeoutID);
    if (this.isPaused || !this.timeoutID) {
      // DEBUG top.console.log("Cannot pause the SuperTimeout because this.isPaused = " + this.isPaused + " OR !this.timeoutID = " + (!this.timeoutID));
  ...
  // Reset the timeout
  _reset() {
    // DEBUG top.console.log("_reset() fired in parent");
    if (this.timeoutID) {
      clearTimeout(this.timeoutID);
  ...
  // Restart the timeout
  restart() {
    // DEBUG top.console.log("restarting...");
    this._reset();
    this._start();

    const index = listOfAllTickingSuperTimers.indexOf(this);
    if (index === -1) {
      // Such a timer doesn't exist in the list » Because it has either completed or was cleared whichever made getLost() fire
      listOfAllTickingSuperTimers.push(this); // MUST: Reenter the list now that restart() has fired
    } else {
      // Such a timer already exists in the list » Because it is still ticking
      // Do nothing
    }
  }
  ...
  clear() {
    // DEBUG top.console.log("clearing... » fired by parent method");
    this._reset();
    this._getLost();
  }
...
class SuperInterval extends SuperTimeout {
  ...
  _start() {
    ...
    if (this.intervalID) {
      // Only _reset() will set intervalID back to null » Until then _start cannot fire to create redundant intervals
  ...
  _set() {
    ...
    this.intervalID = setInterval(() => {
      // DEBUG top.console.log("TICK HAPPENED and before callback this.intervalID = " + this.intervalID);
      this.callback(); // Execute the callback function
  ...
  pause() {
    // DEBUG top.console.log("Overrider pause method of SuperInterval fired. " + "Now this.intervalID = " + this.intervalID);
    if (this.isPaused || !this.intervalID) {
      // DEBUG top.console.log("Cannot pause because this.isPaused = " + this.isPaused + " OR " + " !this.intervalID = " + (!this.intervalID));
  ...
  resume() {
    // DEBUG top.console.log("Overrider resume() fired!");
    if (!this.isPaused || !this.intervalID) {
      // DEBUG top.console.log("Cannot resume because !this.isPaused = " + (!this.isPaused) + " OR " + " !this.intervalID = " + (!this.intervalID));
  ...
  // super magic
  _reset() {
    // DEBUG top.console.log("Overrider _reset() fired!");
    super._reset(); // Perform the same resetting procedure in SuperTimeout
    if (this.intervalID) {
      // With the addition of the task of removing the interval
      clearInterval(this.intervalID);
